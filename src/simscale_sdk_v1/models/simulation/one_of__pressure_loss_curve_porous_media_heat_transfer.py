...
_ONE_OF__PRESSURE_LOSS_CURVE_POROUS_MEDIA_HEAT_TRANSFER_VARIANTS: dict[str, type] = {
    "FLUID_ONLY_HEAT_TRANSFER": FluidOnlyHeatTransfer,
    "EFFECTIVE_CONDUCTIVITY_HEAT_TRANSFER": EffectiveConductivityHeatTransfer,
}

OneOf_PressureLossCurvePorousMediaHeatTransfer = Annotated[
    Union[FluidOnlyHeatTransfer, EffectiveConductivityHeatTransfer],
    BeforeValidator(
        partial(
            parse_discriminated_union,
            disc_key="type",
            variants=_ONE_OF__PRESSURE_LOSS_CURVE_POROUS_MEDIA_HEAT_TRANSFER_VARIANTS,
        )
    ),
]
