...
_ONE_OF__FORCE_MOMENT_COEFFICIENTS_RESULT_CONTROL_WRITE_CONTROL_VARIANTS: dict[str, type] = {
    "TIME_STEP": TimeStepWriteControl,
    "OUTPUT_TIME": SynchronizeWithFieldOutputWriteControl,
}

OneOf_ForceMomentCoefficientsResultControlWriteControl = Annotated[
    Union[TimeStepWriteControl, SynchronizeWithFieldOutputWriteControl],
    BeforeValidator(
        partial(
            parse_discriminated_union,
            disc_key="type",
            variants=_ONE_OF__FORCE_MOMENT_COEFFICIENTS_RESULT_CONTROL_WRITE_CONTROL_VARIANTS,
        )
    ),
]
