...
class TwoResistorNetwork(SimScaleModel):
    ...
    type_: str = Field(
        ...
        description="A Thermal Resistance Network can be used to approximate the effect of heat sources and heat transfer from that source to the surrounding domain without resolving the source geometry.  Select the top face of the body you want to assign. The models for Thermal resistance network are as follows:Star Network Resistance Model: defines a thermal resistance network consisting out of a top, a board, a board to interface and four side resistances. A power source is assigned to the body.Two resistor Model: defines a thermal resistance network consisting out of a top, a board and a board to interface resistance. A power source is assigned to the body.  Learn more.  Schema name: TwoResistorNetwork",
    )
