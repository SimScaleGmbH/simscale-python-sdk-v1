...
class HeatExchangerSource(SimScaleModel):
    ...
    type_: str = Field(
        ...
        description="Power sources can be used to simulate heat generation from a volume. Three types are available:Absolute: Used when total power emitted by the assigned volume is known.Specific: Used when power density of the assigned volume is known.Heat exchanger: Used to model a heat exchanger on a fluid region. The heat input is computed from the total conductance (U [W/K]) and the difference between the fluid temperature (T) and the heat exchanger temperature (Tref). Q = U (T - Tref). Learn more.  Schema name: HeatExchangerSource",
    )
    name: str | None = Field(default=None)
    heat_exchanger_mode: OneOf_HeatExchangerSourceHeatExchangerMode | None = Field(
        validation_alias="heatExchangerMode", serialization_alias="heatExchangerMode", default=None
    )
